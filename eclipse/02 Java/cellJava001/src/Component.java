...
class Component{
  ...
  Component(Voxelgrid _voxelgrid, Agent a){
    ...
    if(PApplet.abs(a.normal.x)>PApplet.abs(a.normal.y) ^ componentsInPlane) dir = new Vec3D(voxelgrid.voxelSize[0]*0.5f,0,0);
  ...
  public void findFacevertices(){
    ...
    if(PApplet.abs(valuesZ.get(0)-valuesZ.get(1))<0.001f){ once = valuesZ.get(2); twice = valuesZ.get(0);}
    if(PApplet.abs(valuesZ.get(0)-valuesZ.get(2))<0.001f){ once = valuesZ.get(1); twice = valuesZ.get(0);}
    if(PApplet.abs(valuesZ.get(1)-valuesZ.get(2))<0.001f){ once = valuesZ.get(0); twice = valuesZ.get(1);}
