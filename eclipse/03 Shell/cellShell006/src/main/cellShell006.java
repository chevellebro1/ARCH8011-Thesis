...
public class cellShell006 extends PApplet {
...
// VOXELS AND COMPONENTS
	...
	boolean componentsAlternating = false;// places components alternatingly in two direction. Overwrites componentsInPlane
	boolean componentsInPlane = false;// the components are either placed within the plane of the agents, or orthogonal to it
...
// GENERIC AGENT SETTINGS (Can be overridden in the Agent's constructor for separate groups of Agents.)
	...
	Vec3D _unary = new Vec3D(0.0f, 0.0f, 0.05f);// unary force (-0.005)
...
// VISUALIZATION
	...
	public void loadFiles() {
		...
		randomSeed(2);
		for (int i = 0; i < 10; i++) {
			Vec3D pos = new Vec3D(random(-5.0f, 5.0f), random(100.0f, 110.0f), random(0, 5.0f));
			Vec3D vel = new Vec3D();
			// Vec3D vel = new Vec3D(random(-0.01,0.01), random(-0.01,0.01),
			// random(-0.01,0.01));
			Agent a = new Agent(pos, vel);
...
//IMPORT POINTS
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "main.cellShell006" };
		if (passedArgs != null) {
