...
// VARIABLES
	...
	ArrayList<Vec3D> agentsMove = new ArrayList<Vec3D>();
...
// GENERIC AGENT SETTINGS (Can be overridden in the Agent's constructor for separate groups of Agents.)
	...
	float _facNeighborsFar = 0.0f;// attraction/repulsion to far neighbors (0.0f)
...
//	CAMERA
	...
	public void settings() {
		...
		if (resolution == "1080") {
			size(1920, 1080, P3D); // 1080
		}
		if (resolution == "4K") {
			size(4000, 2160, P3D); // 4K
		}
	}
	...
	public void loadFiles() {
		...
		randomSeed(0);
		for (int i = 0; i < 10; i++) {
			Vec3D pos = new Vec3D(random(-5.0f, 5.0f), random(-5.0f, 5.0f), random(0.0f, 5.0f));
			Vec3D vel = new Vec3D();
	...
	class Agent extends Vec3D {
		...
		//float[] stepSize;
		float[] stepSize = new float[]{10,10,10};
		...
		public Vec3D followMesh(Mesh mesh, float strength) {
		    ...
		    if(location.b==true) strength=(float) (strength*0.1);
		...
		public void displayTrails() {
			for (int i = 0; i < trail.size()-1; i++) {
				...
				posA = new Vec3D((posA.x/stepSize[0])*stepSize[0], (posA.y/stepSize[1])*stepSize[1], (posA.z/stepSize[2])*stepSize[2]);
				posB = new Vec3D((posB.x/stepSize[0])*stepSize[0], (posB.y/stepSize[1])*stepSize[1], (posB.z/stepSize[2])*stepSize[2]);
	...
	class Mesh {
		...
		public Vec3D closestPoint(Vec3D point){
		  ...
		  public Vec3D_Bool closestLocation(Vec3D point){
		    ...
		    boolean edge = false;
		    if(edgeCommon.faces.size()==1){
		      if(closest.distanceToSquared(cv)<0.001 || closest.distanceToSquared(cv2)<0.001) edge = true;//outer vertex
		      else if(closest.distanceToSquared(edgeCommon.closestPoint(point))<0.001) edge = true;//outer edge 
		    }
		...
		public void drawStructure() {
			for (Edge e : edges) {
				if (e.countTrails > 5) {
					...
					float colorValue = meshFollow.closestVertex(p0).colValue;
...
//PRINT DEBUG VARIABLES
	public void debugPrint() {
		...
		output.println("Agent Index");
		
		output.println(agents.size());

		output.println();

		output.println("Old Index");
		
		output.println(agentsMove);
...
//EXPORT KEYS
	public void keyPressed() {
		...
		if (key == 'p') {
			run = false;
			println("paused.", "camera lookAt:" , cam.getLookAt()[0], ",", cam.getLookAt()[1], ",",
					cam.getLookAt()[2], "camera Distance:" , cam.getDistance(), "camera Rotation:" , cam.getRotations()[0], ",", cam.getRotations()[1], ",",
					cam.getRotations()[2], "camera Position:" ,cam.getPosition()[0], ",", cam.getPosition()[1], ",", cam.getPosition()[2]);
		}
		if (key == 'r') {
			run = true;
			println("running");
