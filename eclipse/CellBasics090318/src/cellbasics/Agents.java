...
class Agent extends Vec3D {
 ...
 private float _drag;
 private float _facNeighborsClose;
 private float _facNeighborsClosest;
 private float _facNeighborsFar;
 private float _facPlanarize;
 private float _facStrata;
 private float _facOrthogonal;
 private float _facAttractors;
 private Vec3D _unary;
 private float _facFollowMesh;
 private float _facVoxel;
 private int _minAge;
 private int _countDivide;
 private float _rangeDivide;
 private float _offsetDivision;
 private float _facVelChild;
 private float _facVelParent;
 private float _rangeClose;
 private ArrayList<Attractor> attractors;
 private ArrayList<Attractor> agents;
 
 private float random(float f, float offsetDivision2) {
 ...
 void move(){
   ...
   if(age>minAge){
     ...
     if(count>0){
       ...
       distanceAverage=distanceAverage/(count);
...
void update(){
   //UPDATE
   vel.scaleSelf((float) (1.0-drag));// apply the drag to the previous velocity
   vel.addSelf(acc);// add the acceleration to the velocity
   if(age>250) vel.scaleSelf((float) 0.2);//slower movement for older agents
 ...
 Vec3D followMesh(float strength) {
   ...
   if(edgeCommon.faces.size()>1){
     ...
     direction.scaleSelf(0.1);//----------------------------------------smaller scale factor for the mesh edges to reduce sticking
