...
public class Karamba {
	...
	public void getDisplacements(ArrayList<cellJava005.Agent> agents) {
		...
		for (Agent a : points) {// loop through points, not agents
			for (Agent n : a.neighbors) { // agents for neighbor
				if (a.index < n.index || n.neighbors.contains(a) == false) {// agents can have each other as neighbors.
																			// therefore in order to avoid duplicating
																			// elements, beams are only added if the
																			// index of a is smaller, or if the neighbor
																			// does not have the agent in its list of
																			// neighbors
					nodePairs.add(
							new ArrayList<feb.Node>(Arrays.asList(nodes[points.indexOf(a)], nodes[points.indexOf(n)])));
				}
			}
		}
