...
// GENERIC AGENT SETTINGS (Can be overridden in the Agent's constructor for separate groups of Agents.)
	...
	Vec3D _unary = new Vec3D(0.0f, 0.0f, 0.03f);// unary force (-0.005)
...
//	CAMERA
	...
	public void setup() {
		...
		if (defaultCam == true) {
			...
			cam = new PeasyCam(this, -97.05684 , -124.44902 , 31.330812, 100); // saved view
			cam.setRotations(1.3241655 , -0.64751655 , 2.7723508); // saved view
		}
		...
		public void move() {
			...
			if (age < 250)
				col = new int[] { PApplet.parseInt(strokeStart[0] + (age * red)), PApplet.parseInt(strokeStart[1] - (age * green)), 
						PApplet.parseInt(strokeStart[2] + (age * blue)) , 255 };
			findNeighbors();
			normal = findNormal();
