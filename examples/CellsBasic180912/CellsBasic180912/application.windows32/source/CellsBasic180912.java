...
public class CellsBasic180912 extends PApplet {
  ...
  Component(Voxelgrid _voxelgrid, Agent a){
    ...
    if(voxelType == 0){//rectangular grid
      if(componentsAligned){
        if(axisComponentAlignment==0) dir = new Vec3D(voxelgrid.voxelSize[0],0,0);
        else if(axisComponentAlignment==1) dir = new Vec3D(0,voxelgrid.voxelSize[1],0);
        ...
        if(componentsAlternating){
          ...
          if(densities.size()>0){
            ...
            if(density==1) col = new int[]{0,80,130};
            else if(density==2) col = new int[]{50,120,170};
            else if(density==3) col = new int[]{100,160,210};
...
public void getDisplacements(){
  ...
  for(int i=0;i<agents.size();i++) agents.get(i).displacement = displacements.get(i);
  println("displacements imported:",displacements.size());
}
  public void settings() {  size(1280, 720, P3D);  smooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CellsBasic180912" };
    if (passedArgs != null) {
