...
class Mesh {
  ...
  Vertex closestVertex(Vec3D point){
  ...
  Vec3D_Bool closestLocation(Vec3D point){
    Vertex cv = closestVertex(point);//closest vertex
    Vertex cv2 = null;// the second closest vertex among cv's neighbors
    for(Vertex x:cv.vertices) cv2=(cv2==null||x.distanceToSquared(point)<cv2.distanceToSquared(point))?x:cv2;
    Edge edgeCommon = null;//find the common edge between the points
...
class Face {
    ...
    if ( s + t < det ){
      ...
      if ( s < 0.0 ){
        ...
        if ( tmp1 > tmp0 ){
          ...
          float denom = a-2*b+c;
          s = Math.max(0.0, Math.min(1.0, numer/denom));
          t = 1-s;
        }else{
          t = Math.max(0.0, Math.min(1.0, -e/c));
          s = 0.0;
        ...
        if ( a+d > b+e ){
          ...
          s = Math.max(0.0, Math.min(1.0, -e/c));
          t = 0.0;
        }
      }else{
        float numer = c+e-b-d;
  ...
  Vec3D pointInPlane(Vec3D point){
